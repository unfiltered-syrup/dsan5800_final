...
def draw_single_plot(plot_cfg: Dict[str, Any], idx: int):
    ...
    if chart_type == "bar":
        if x_field not in df.columns or y_field not in df.columns:
            print(
                f" Plot {idx}: bar chart needs x_field={x_field} and "
    ...
    # LINE
        ...
        if x_field not in df.columns or y_field not in df.columns:
            print(
                f" Plot {idx}: line chart needs x_field={x_field} and "
                f"y_field={y_field} in columns."
            )
            plt.close(fig)
            return
    ...
    # SCATTER
        ...
        if x_field not in df.columns or y_field not in df.columns:
            print(
                f" Plot {idx}: scatter chart needs x_field={x_field} and "
                f"y_field={y_field} in columns."
            )
            plt.close(fig)
            return
    ...
    # HISTOGRAM
        ...
        if col not in df.columns:
            print(
                f" Plot {idx}: histogram needs a numeric column '{col}' "
                "in the result."
            )
            plt.close(fig)
            return
    ...
    # BOX
        ...
        if col not in df.columns:
            print(
                f" Plot {idx}: boxplot needs a numeric column '{col}' "
                "in the result."
            )
            plt.close(fig)
            return
    ...
    # PIE
        ...
        if x_field not in df.columns or y_field not in df.columns:
            print(
                f" Plot {idx}: pie chart needs x_field={x_field} and "
                f"y_field={y_field} in columns."
            )
            plt.close(fig)
            return
...
def visualize_from_plan(plan: Dict[str, Any]):
    ...
    if not plots:
        print(" No plots proposed in VIS_PLAN_JSON.")
        return
